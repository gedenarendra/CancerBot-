...
//           style={{
//             position: "fixed",
//             bottom: "20px", // Sedikit lebih bawah
//             right: "60px", // Sedikit lebih kanan (menjauhi tengah layar)
//             zIndex: 100,
...
// ==========================================
// ✅ 1. DEFINISI EFEK DAUN (YANG TADI HILANG)
// ==========================================
// Kita taruh di luar component agar posisinya statis & tidak berkedip saat ngetik
const leaves = Array.from({ length: 15 }).map((_, i) => ({
  id: i,
  left: `${Math.random() * 100}%`,
  animationDelay: `${Math.random() * 10}s`,
...
// ==========================================
// 2. COMPONENT UTAMA (USER DASHBOARD)
// ==========================================
const User = ({ token, userId, username, handleLogout }) => {
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await axios.post(`${API_AI}/ask`, { query: currentInput });
      const aiReply = res.data.answer;
      const aiTime = new Date().toISOString();
      setMessages((prev) => [
        ...prev,
